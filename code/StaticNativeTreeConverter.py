...
class StaticNativeTreeConverter(StaticNativeTreeConverter):
    def __init__(self, dim, namespace, featureType):
            super().__init__(dim, namespace, featureType)
    ...
    def getImplementation(self, head, treeID):
            ...
            while len(nodes) > 0:
                ...
                # probChild becomes either the left or the right child of the root
                if head.probLeft is not None: # when there is leftChild
                        if head.probRight is not None: # when there is rightChild too
                                if head.probLeft > head.probRight:
                                        entry.append(head.leftChild)
                                        # print('Left child:', nextIndexInArray-2)
                                else:
                                        entry.append(head.rightChild)
